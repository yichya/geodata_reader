...
impl eframe::App for GeoDataReader {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::Window::new("GeoIP").show(ctx, |ui| {
            let gm: &Download<Vec<geodata::GeoIp>> = &self.geoip.lock().unwrap();
            match gm {
                Download::Done(r) => match r {
                    Ok(geoip) => {
                        egui::SidePanel::right("geoip_right_panel").show_inside(ui, |ui| {
                            ui.label(format!(
                                "{} Total: {}",
                            ...
                            egui::ScrollArea::vertical().show(ui, |ui| {
                                for (index, element) in geoip.iter().enumerate() {
                                    if element.country_code.contains(
                                        self.country_code_search.to_ascii_uppercase().as_str(),
                                    ) {
                                        for ele2 in element.cidr.iter() {
                                            let mut country_code_match = false;
                                            match ele2.to_ipnet() {
                                                Ok(n) => match self.ip_search.parse::<IpAddr>() {
                                                    Ok(ip_search_parsed) => {
        ...
        egui::Window::new("GeoSite").show(ctx, |ui| {
            ...
            match gm {
                Download::Done(r) => match r {
                    Ok(geosite) => {
                        egui::SidePanel::right("geosite_right_panel").show_inside(ui, |ui| {
                            ui.label(format!(
                                "{} Total: {}",
